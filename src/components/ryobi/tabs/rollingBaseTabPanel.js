...
const RollingBaseTabPanel = ({
  ...
  return (
    <TabPanel
      value={value}
      index={index}
      className={"reduce-padding card-body-content " + fullHeightClass}
    >
      <div className="tab-pane" id="mobile_build" role="tabpanel">
        <div className="top_product_area dashed_border-bottom mobile_bulder row m-0 top_products_intro">
          <h6 className="font-size-small  d-block col-sm-12 ">Rolling Base</h6>
          {rollingBase.map(
            ({
              id,
              imageName,
        ...
        <div className="products_area pt-3">
          <div className="products_block">
            <div className="row  margin_removed pb-2">
              {!mobileItems.length && (
                <div className="item-block-overlay">
                  Please select the Base to get started with your Mobile Build.
                </div>
              )}
              {rollingBaseItem.map((rolling, i) => {
                ...
                if (mobileTopProduct) {
                  const mobileTopProductData = mobileAssets.find(
                    (i) => i.id === mobileTopProduct.itemId
                  );
                  isAllowed =
                    mobileTopProductData.allowed.includes(rolling.id) && // check if other products are allowed to place on top of it
                    (mobileTopProduct.isStackRowComplete ||
                      mobileTopProductData.allowedPartials?.includes(
                        rolling.id
                      )); // check if top row complete
                }
                return (
                  <MobileItemContainer
                    isAllowed={isAllowed}
            ...
            {!!wallInMobileItems.length && (
              <div className="row  margin_removed dashed_border-top pt-3">
                ...
                {wallInMobileItems
                  .map((el) => ({
                    ...el,
                    ...displayItems.find((el2) => el2.id === el.itemId),
                  }))
                  .map((item, i) => (
                    <div className="col-sm-4 col-6" key={i.toString()}>
        ...
        {!isMobile && (
          <div
            ...
            className="add_products_btn show_tour_btn"
          >
            show tour
          </div>
        )}
        <div
          className="reset font-size-small"
          style={{ opacity: mobileItems.length ? 1 : 0.5 }}
        >
          <div
            onClick={() =>
              mobileItems.length ? setNotifyModal("resetMobile") : null
            }
            style={{ cursor: mobileItems.length ? "pointer" : "default" }}
          >
